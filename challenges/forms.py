...
class RevealHintForm(forms.Form):
    ...
    def clean(self):
        ...
        if (team is not None
        and hint in team.hints.all()):
            raise forms.ValidationError(
                'Your team has already revealed this hint!'
            )
        if is_solved(hint.challenge, team):
            raise forms.ValidationError(
...
class SubmitFlagForm(forms.Form):
    ...
    attempted_flag = forms.CharField(
    ...
    def clean(self):
        ...
        if is_locked(challenge, self.player):
            raise forms.ValidationError(
                'This level is currently locked '
                'or the CTF is not currently active!'
            )
